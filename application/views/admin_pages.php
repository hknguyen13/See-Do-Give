...
{
 mysql_query("insert into adminpages (PageName,PageTitle,PageMetaTitle,PageMetaKeywords,PageMetaDescription) values ('".addslashes($_REQUEST['pagename'])."','".addslashes($_REQUEST['pagetitle'])."','".addslashes($_REQUEST['metatitle'])."','".addslashes($_REQUEST['metakeywords'])."','".addslashes($_REQUEST['metadescription'])."')");
	
	$result=mysql_query("select max(PageId) as pageid from adminpages");
	
	while($row = mysql_fetch_array($result))
	  {	  
	$query = mysql_query("insert into pagecontent (PageContent,PageContentPageId) values ('".addslashes($_REQUEST['elm1'])."',".$row['pageid'].")");
	}
...
<table class="product_table">
          <tr>
            ...
            <td><input type="text" id="pagename" name="pagename" size="80" value="" ></td>
          </tr>
          <tr>
            <td>Page Content</td>
            <td><textarea id="elm1" name="elm1" rows="15" cols="80" style="width: 95%"></textarea></td>
          </tr>
          
          <tr>
            <td><input type="hidden" id="edit" name="edit" value="" />
            <input type="hidden" id="pageId" name="pageId" value=""/></td>
