...
/*	$x = $this->db->query("select * from user_tbl where facebook_id='$uid'");
	$y = $x->result();
	$data['userid'] = $y['0']->userid;*/
	
	$query = $this->db->query("select * from user_tbl where facebook_id='$id'");
	$result = $query->result();
	if(count($result)==0) 
	{
	$this->db->query("insert into user_tbl (facebook_id,userfirstname,	userlastname,useremail,userpassword,usergender,userdob,userphone,usercountry,userstate,userstatus) values ('$id','$name','','$email','','','$birthday','','','','1')");
	
	$userid = $this->db->insert_id();
	}
	
	$data['userid']=$userid;
	$x = $this->db->query("select * from user_tbl where userid='$userid'");
	$y = $x->result();
	$data['name'] = $y['0']->userfirstname;
	if($this->uri->segment("3"))
	{
	//print_r($data['userid']); exit;
	$videoid = $this->uri->segment("3");
	$qry = $this->db->query("select * from broadcastervideo where bcbroadcaster_id='$videoid'");
	$rst = $qry->result();
	$data['bc_id'] = $rst['0']->bcbroadcaster_id;
	...
	*/
	...
	function test() {
	 $v = $this->session->set_userdata('fb_data');
	 print_r($vf);
	}
